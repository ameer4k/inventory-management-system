...
max=$(dialog --stdout --inputbox "Enter The Max Quantity Considered Low-Stock:" 8 60)
((max=max+1))
    ...
    dialog --textbox ./LsReport.txt 60 120
...
genSalesReport () {
    ...
    date1=$(dialog --stdout --inputbox "Enter The Start Date (in format yyyy-mm-dd):" 8 60)
    ...
    for i in $(seq 1 "$lines");
    do
        sellingDate=$(sed -n "${i}p" ./sales.txt | cut -d',' -f1)          
...
displayTopSelling () {
    ...
    topK=$(dialog --stdout --inputbox "How The Number of Top Medicine To Show:" 8 60)

    sort -t',' -k2 -nr ./salesPerMedName.txt | head -n "$topK" > ./LsReport.txt 
